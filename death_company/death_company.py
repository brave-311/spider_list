    ...
    try:
        ...
        if json_data.status_code is 200:
            ...
            for data in deatail_infos:
                ...
                com_people = ''.join(
                    name['name'] + '/' + name['per_des'] + ';'
                    for name in com_team
                )

                # 公司标签
                com_tag = data['com_tag']
                com_tags = ''.join(tag['tag_name'] + '/' for tag in com_tag)
                # 投资轮次
                com_fund_status_name = data['com_fund_status_name']
                # 投资公司
                com_invst = data['com_invst']
                com_invsts = ''.join(com['invst_name'] + '/' for com in com_invst)
                # 公司死亡原因
                closure_type = data['closure_type']
                death_reason = ''.join(da['name'] + '/' for da in closure_type)
                # 公司存活时间
                ...
                try:
                    ...
                    db.commit()
                    print(f'{com_name}数据写入成功！！！')
                except:
                    print('数据写入失败！')
                    db.rollback()
        else:
            print(f'{json_url}访问不了了！！！')
    except:
        print(f"{json_url}数据爬取失败")

if __name__ == "__main__":
    start = time.time()
    print(f'当前环境CPU核数是：{multiprocessing.cpu_count()}核')
    json_urls = [
        f'https://www.itjuzi.com/api/closure?com_prov=&sort=&page={i}&keyword=&cat_id='
